...
> **인증 한 줄 요약** — 로컬 계정(bcrypt) · Discord OAuth · Google OAuth, 세 가지를 함께 켤 수 있다. 백엔드가 단일 출처이고, OAuth 자격증명은 환경 변수가 아니라 관리자 화면에서 런타임에 DB 에 저장하므로 프론트엔드를 다시 빌드할 필요가 없다.
...
### 로그인 — Discord + Google OAuth + 로컬 한 화면
...
`AUTH_TYPE` 설정에 따라 로컬 / OAuth / 둘 다 노출된다. OAuth 자격증명을 관리자 화면에서 등록하면 그 즉시 위와 같이 Discord/Google 버튼이 로컬 폼 위에 함께 나타난다. 프론트엔드 재빌드 없음.
