...
app.get("/copy", async (req, res) => {
  ...
  await insertLog({
    token,
    activity: "COPY",
    description: `COPY [FILE] FROM /${convertLoc(
      `${originLoc}/${fileName}`
    )} TO /${convertLoc(`${targetLoc}/${fileName}`)}`,
    loc: `/${targetLoc}`,
    time: Date.now(),
  });
...
app.get("/move", async (req, res) => {
  ...
  await insertLog({
    token,
    activity: "MOVE",
    description: `MOVE [FILE] FROM /${convertLoc(
      `${originLoc}/${fileName}`
    )} TO /${convertLoc(`${targetLoc}/${fileName}`)}`,
    loc: `/${targetLoc}`,
    time: Date.now(),
...
app.get("/rename", async (req, res) => {
  ...
  await insertLog({
    token,
    activity: "RENAME",
    description: `RENAME [FILE] AT /${convertLoc(
      `${loc}/${name}`
    )} TO /${convertLoc(`${loc}/${change}`)}`,
    loc: `/${loc}`,
    time: Date.now(),
  });
...
app.post("/zipFiles", async (req, res) => {
  ...
  output.on("close", () => {
    fsNP.writeFileSync(
      progressPath,
      JSON.stringify({ percent: 100, status: "done" })
    );
  ...
  archive.on("error", (err) => {
    fsNP.writeFileSync(
      progressPath,
      JSON.stringify({ percent: 100, status: "error" })
    );
  ...
  archive.on("entry", () => {
    ...
    fsNP.writeFileSync(
      progressPath,
      JSON.stringify({ percent, status: "zipping" })
    );
  });
...
app.post("/unzipFile", async (req, res) => {
  ...
  const extractTo = path.join(
    basePath,
    file.loc,
    path.parse(file.name).name + "_unzipped"
  );
      ...
      zipfile.on("entry", async (entry) => {
        ...
        if (/\/$/.test(entry.fileName)) {
          ...
          fsNP.writeFileSync(
            progressPath,
            JSON.stringify({
              percent: Math.floor((processedEntries / totalEntries) * 100),
              status: "unzipping",
            })
          );
          zipfile.readEntry();
...
app.get("/img", async (req, res) => {
  ...
  if (check) {
    ...
    const img = await fs.readFile(
      path.resolve(__dirname, "..", "img", `file.png`)
    );
    res.end(img);
  }
});
...
app.get("/kakaoLogin", async (req, res) => {
  try {
    ...
    // Validate Kakao OAuth configuration
    if (!KAKAO_REST_API_KEY || !KAKAO_REDIRECT_URL || !KAKAO_CLIENT_SECRET) {
      console.error("❌ Kakao OAuth not configured properly");
      res.status(500).json({
...
app.post("/auth/register", async (req, res) => {
  const { userId, username, password, krname } = req.body;
  ...
  const result = await AuthModule.registerLocalUser(
    userId,
    username,
    password,
    krname
...
app.post("/auth/login", async (req, res) => {
  ...
  const result = await AuthModule.loginLocal(userId, password);
...
const server = app.listen(PORT, async () => {
  ...
  // Run migrations
  const { migrateLocalAuth } = require("./migrations/addLocalAuth");
  migrateLocalAuth();
