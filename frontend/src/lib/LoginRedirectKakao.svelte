<script lang="ts">
    ...
    interface userdata_kakao{
        userId: string;
        nickname: string;
    }

    let register:boolean = false;
    let saveData:userdata_kakao = {
        userId: '',
        nickname: ''
    }
    ...
    onMount(async () => {
        access_code = location.href.split("code=")[1];

        const loginRES = await fetch(`/server/kakaoLogin?code=${encodeURIComponent(access_code)}`);
        ...
        if(res.status == "new"){
            ...
            saveData.nickname = res.nickname;
        }else{
            useAuth.set({
                userId: res.userId,
                username: res.nickname,
                krname: res.krname,
                global_name: res.nickname,
                token: res.token
            })
    ...
    async function goregister() {
        ...
        const res = await fetch(`/server/registerKakao`, {
            method:"POST",
            headers:{
        ...
        if(data.status == "complete"){
            useAuth.set({
                userId: saveData.userId,
                username: saveData.nickname,
                global_name: saveData.nickname,
                token: data.token,
                krname: koreanName
            })
            const baseUrl = `${window.location.protocol}//${window.location.host}/`;
...
<main>
    ...
    {:else}
        <div id="registerCon">
            ...
            <div class="inputCon">
                <div class="inputTitle">Your kakao id</div>
                <input type="text" value="{saveData.userId}" readonly class="noEdit">
            </div>
            <div class="inputCon">
                <div class="inputTitle">Your kakao nickname</div>
                <input type="text" value="{saveData.nickname}" readonly class="noEdit">
            </div>
            <div class="inputCon">
